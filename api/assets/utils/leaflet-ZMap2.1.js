        ...
        z.AddFeatureLayer.prototype={
            ...
            _isInCurExtent:function(dataListi,extent){
                if(this._featureType.toLowerCase()=='marker'||this._featureType.toLowerCase()=='circlemarker'){
                    ...
                    let pathsString = dataListi.SHAPE || dataListi.POINTS||dataListi.shape;
                    let linePaths=this._stringLatlngToArray(pathsString);
                    if(linePaths.length>0){
                        for(let i=0;i<linePaths.length;i++){
                            if (linePaths[i][0] > extent.minLat && linePaths[i][0] < extent.maxLat && linePaths[i][1] < extent.maxLng && linePaths[i][1] > extent.minLng) {
            ...
            _stringLatlngToArray:function(str){
                ...
                pointsStr=pointsStr.replace('MULTILINESTRING','');
    ...
    (function(z){
        z.Marker=L.Marker.extend({
            initialize: function(latlng, options) {
                ...
                let opt = {};
                let myIcon = null;
                if(this.iconOpt.icon!=undefined&&(this.iconOpt.icon instanceof L.Icon||this.iconOpt.icon instanceof L.DivIcon)){
